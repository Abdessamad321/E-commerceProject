...
const Shop = () => {
  ...
  const [priceRangeFilter, setPriceRangeFilter] = useState({ min: 0, max: 20000 });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:7000/v1/allproducts?query=${searchQuery}`);
  ...
  return (
    <div className="mt-4">
      ...
      <label style={{ display: 'grid' }}>
          <p className="tittle" style={{ marginBottom: '10px', fontSize: '1rem', display: 'flex', justifyContent: "space-between", alignItems: 'center' }}>
            Sort By <FaSort />
          </p>
          <select
            className="selects"
            style={{  padding: '10px 5px', borderRadius: '16px', marginBottom: '20px', fontSize: '1rem' }}
            value={sortBy}
        ...
        <div style={{ display: "grid" }} controlId="priceRangeFilter">
          <label style={{ display: 'grid' }}>
            <p className="tittle" style={{ marginBottom: '10px' }}>Price Range Filter</p>
            <input
      ...
      <div style={{ display: 'grid', width: '80%' }}>
        {noResults ? (
            <div className="no-results-message" style={{textAlign: 'center'}}>
              ...
              <p style={{ fontSize: '3rem'}}>No products found for "{searchQuery}"</p>
            </div>
          ) : (
        <div className="product-list">
          {filterProducts(sortProducts())
            .filter((product) => currentCategory === "all" || product.categoryName === currentCategory)
            ...
            .map((item) => (
              <div className="card" key={item._id} >
                ...
                <div className="imageprdcts" onClick={() => navigateToProductDetail(item._id)}>
                  <img className="product--image" src={item.product_image} alt="product image" />
                </div>

                <div className="cart-text" onClick={() => navigateToProductDetail(item._id)}>
                  <span className="prdctname">{item.product_name}</span>
                  <p className="ellipsis">{item.short_description}</p>
                  <p className="price">
                    <span style={{ fontSize: "1.5em", color: '#590404' }}>${Math.floor(item.price)}</span>
                    <span style={{ fontSize: "1em" }}>.{Math.floor((item.price % 1) * 100)}</span>
                  </p>
                </div>
                ...
                <div className="addbutton">
                  ...
                  >
                    <MdAddShoppingCart style={{ fontSize: "1.3rem" }} />
                    Add To Cart
                  </Button>
                </div>
              </div>
        ...
        {visibleProducts < filterProducts(sortProducts()).length && (
          <Button
            onClick={() => setVisibleProducts((prevVisible) => prevVisible + 8)}
            style={{
              margin: "20px auto",
              padding: "10px 20px",
              borderRadius: "8px",
