...
function Categories() {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:7000/v1/categories?page=${currentPage}&perPage=${itemsPerPage}`);
  ...
  const handleUpdate = async (updatedData, categoryId) => {
    try {
      const response = await axios.put(
        `http://localhost:7000/v1/categories/${categoryId}`,
        updatedData
      );
  ...
  const removeCategory = async (categoryId) => {
   ...
   const handleSearch = async (value) => {
     ...
     try {
       ...
       const response = await axios.get('http://localhost:7000/v1/Categories', {
         params: {
           query: value,
         },
       });
