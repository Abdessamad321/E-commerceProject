...
const CustomersList = () => {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:7000/v1/customers?page=${currentPage}&perPage=${itemsPerPage}&sort=${sortDirection}`
      );
  ...
  const updateCustomerEndpoint = (customerId) =>
    `http://localhost:7000/v1/customers/${customerId}`;
  ...
  const searchCustomersEndpoint = "http://localhost:7000/v1/customers";
