...
const OrderDetailsModal = ({ orderDetails, handleClose }) => (
  <Modal show={orderDetails !== null} onHide={handleClose}>
    ...
    {orderDetails && (
      <>
        <p><span style={{fontWeight:'800'}}>Order Items:</span>  {orderDetails.order_items}</p>
...
const Orders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:7000/v1/orders?page=${currentPage}&perPage=${itemsPerPage}`
      );
  ...
  const updateOrderEndpoint = (orderId) =>
    `http://localhost:7000/v1/orders/${orderId}`;
