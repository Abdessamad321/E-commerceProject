...
function Subcategories() {
  ...
  const fetchSubcategories = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:7000/v1/Subcategories?page=${currentPage}&perPage=${itemsPerPage}`);
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    try {
      const response = await axios.post(
        "http://localhost:7000/v1/Subcategories",
        newSubcategory
      );
  ...
  const handleUpdate = async (updatedData, subcategoryId) => {
    try {
      const response = await axios.put(
        `http://localhost:7000/v1/Subcategories/${subcategoryId}`,
        updatedData
      );
  ...
  const removeSubcategory = async (subcategoryId) => {
   ...
   const handleSearch = async (value) => {
     ...
     try {
       ...
       const response = await axios.get('http://localhost:7000/v1/Subcategories', {
         params: {
           query: value,
         },
       });
