...
function validateProducts (product_name, shortDescription, longDescription, price, quantity ){
const errors = []
...
if(!price || isNaN(Number(price)) || Number(price) <= 0 ||!Number(price.match( /^\d+\.\d{2}$/))){
  errors.push('Price must be a number greater than zero and contain exactly two decimal places');
}
