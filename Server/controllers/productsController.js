...
async function createProduct(req, res) {
    try {
        const { sku, productName, subcategoryID, shortDescription, longDescription, price, quantity, discountPrice, options } = req.body;
        const skuu = xss(sku);
        ...
        const validationErrors = validateUserInput.validateProducts(productName,shortDescription, longDescription, price, quantity );
        ...
        await creation.save();
        res.status(201).json({ message: `Product successfully created`, product: creation });
    } catch (error) {
...
async function findProducts (req, res){
    try {
        ...
        const perPage = 3;
        const searchAll = await product.aggregate([
            {$skip:(page - 1) * perPage},
            {$limit: perPage},
...
async function getProductById(req, res){
    try {
        ...
        const findPro = await product.aggregate([
            {$match: {_id: productId} },
            {
                $lookup:{
                    from: "subcategories",
...
async function updateProduct(req, res){
    try {
        ...
        const UpdateProduct = await product.findByIdAndUpdate(proId, {product_name:productName, sku: sku})
        if (!UpdateProduct) {
            res.status(404).json('products not found')
        }
        UpdateProduct.save();
